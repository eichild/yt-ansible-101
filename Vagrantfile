...
Vagrant.configure("2") do |config|
  ...
  machines.each do |name, conf|
    config.vm.define "#{name}" do |machine|
      ...
      machine.vm.hostname = "#{name}.yt.example"
    ...
    config.vm.provision "shell", inline: <<-EOF
      ...
      echo '10.10.10.30 ansible.yt.example' >> /etc/hosts
      echo '10.10.10.31 machine01.yt.example' >> /etc/hosts
      echo '10.10.10.32 machine02.yt.example' >> /etc/hosts
      EOF
    end
  end
end